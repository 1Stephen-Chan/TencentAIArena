...
from agent_diy_baseline1.algorithm.algorithm import Algorithm
from agent_diy_baseline1.conf.conf import Config
from agent_diy_baseline1.feature.definition import ActData, ObsData
from agent_diy_baseline1.feature.preprocessor import Preprocessor
from agent_diy_baseline1.model.model import Model
...
class Agent(BaseAgent):
    ...
    def exploit(self, env_obs):
        ...
        if safety.shape[0] == Config.ACTION_NUM and treasure.shape[0] == Config.ACTION_NUM:
            prior_logits = 1.8 * safety + 0.7 * treasure
            min_dist = float(remain_info.get("min_monster_dist", Config.MAP_DIAG))
            if min_dist <= 4.0:
                prior_logits[8:] += 0.25 * safety[8:]

            prior_prob = self._legal_soft_max(prior_logits, legal)
            mix = 0.60 if min_dist <= 4.0 else 0.30
