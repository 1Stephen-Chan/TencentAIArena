...
class EpisodeRunner:
    ...
    def _build_curriculum_conf(self):
        ...
        if ep < 2000:
            ...
            env_conf["treasure_count"] = random.randint(9, int(env_conf.get("treasure_count", 10)))
            ...
            env_conf["max_step"] = max(2000, int(env_conf.get("max_step", 1000)))

        elif ep < 4000:
            env_conf["map"] = [1, 3, 4, 5]
            env_conf["map_random"] = True
            env_conf["treasure_count"] = random.randint(8, int(env_conf.get("treasure_count", 10)))
            env_conf["buff_count"] = max(1, int(env_conf.get("buff_count", 2)))
            env_conf["monster_interval"] = max(650, int(env_conf.get("monster_interval", 300)))
            env_conf["monster_speedup"] = max(800, int(env_conf.get("monster_speedup", 500)))
            env_conf["max_step"] = max(2000, int(env_conf.get("max_step", 1000)))

        elif ep < 8000:
            env_conf["map"] = [1,  3, 4, 5, 6, 8, 9]
            env_conf["map_random"] = True
            env_conf["treasure_count"] = min(6, int(env_conf.get("treasure_count", 8)))
